...
def bot_sys_stats():
    ...
    bmsg = f'______🚀𝕋𝕖𝕝𝕝𝕐 Bot Info______\n\n'
    bmsg += f"C: {cpup}% | "
    bmsg += f"R: {ramp}% | "
    bmsg += f"S: {swap}% | "
    bmsg += f"D: {disk}%\n\n"
    bmsg += f"Bandwidth Used: {traf}\n"
    bmsg += "Join @TellYMirror"
    return bmsg
...
async def stats(_, message, edit_mode=False):
    ...
    bot_stats = f"<b><i><u>🚀𝕋𝕖𝕝𝕝𝕐 Bot Statistics</u></i></b>\n\n"\
                ...
                f'\n\n<a href="https://t.me/TellYMirror">𝑩𝒐𝒕 𝒃𝒚 🚀 𝕋𝕖𝕝𝕝𝕐-𝑴𝒊𝒓𝒓𝒐𝒓</a>'

    sys_stats = f"<b><i><u>🚀𝕋𝕖𝕝𝕝𝕐 System Statistics</u></i></b>\n\n"\
...
async def send_bot_limits(_, query):
    ...
    bot_limit = f"<b><i><u>🚀𝕋𝕖𝕝𝕝𝕐 Bot Limitations</u></i></b>\n" \
